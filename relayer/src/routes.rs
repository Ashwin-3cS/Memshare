...
fn metadata_value(metadata: &MemoryMetadata) -> Result<Value, AppError> {
...
pub async fn remember(
    ...
    let response = store_memory_fact(&state, &auth, owner, text, namespace, &body.metadata).await?;
    tracing::info!(
        "remember complete: blob_id={}, owner={}, ns={}",
        response.blob_id,
        owner,
        namespace
    );
...
///
/// Batch ingest for agent-extracted facts. Each item is embedded and stored
/// independently so recall and restore can preserve fact-level provenance.
pub async fn remember_batch(
    ...
    if body.facts.is_empty() {
        return Err(AppError::BadRequest("facts cannot be empty".into()));
    }

    let owner = &auth.owner;
    let total_text_bytes: i64 = body
        .facts
        .iter()
        .map(|fact| fact.text.as_bytes().len() as i64)
        .sum();
...
pub async fn recall(
    ...
    let hits = state
        .db
        .search_similar(&query_vector, owner, namespace, &body.filters, body.limit)
        .await?;
...
pub async fn remember_manual(
    ...
    let upload = walrus::upload_blob(
        ...
        &state.config.package_id,
        Some(&auth.public_key),
        body.metadata.project_id.as_deref(),
        body.metadata.capsule_id.as_deref(),
        body.metadata.task_id.as_deref(),
    )
    .await?;
    ...
    let id = uuid::Uuid::new_v4().to_string();
    let metadata_json = metadata_value(&body.metadata)?;
    state
        .db
        .insert_vector(
            &id,
...
pub async fn recall_manual(
    ...
    let hits = state
        .db
        .search_similar(&body.vector, owner, namespace, &body.filters, body.limit)
        .await?;
    let total = hits.len();
...
pub async fn analyze(
    ...
    let tasks: Vec<_> = facts.iter().map(|fact_text| {
        ...
        async move {
            ...
            let id = uuid::Uuid::new_v4().to_string();
            let metadata = json!({
                "fact_type": "analyze_extracted",
                "source_tool": "memwal_relayer_analyze",
            });
...
pub async fn ask(
    ...
    let hits = state
        .db
        .search_similar(&query_vector, owner, namespace, &body.filters, limit)
        .await?;
...
pub async fn restore(
    ...
    let on_chain_blobs = walrus::query_blobs_by_owner(
        ...
        body.filters.project_id.as_deref(),
        body.filters.capsule_id.as_deref(),
        body.filters.task_id.as_deref(),
    )
    .await?;
    ...
    let blob_package_ids: std::collections::HashMap<String, String> = on_chain_blobs
        ...
        .collect();
    let blob_metadata: std::collections::HashMap<String, Value> = on_chain_blobs
        .iter()
        .map(|b| {
            (
                b.blob_id.clone(),
                json!({
                    "project_id": if b.project_id.is_empty() { None::<String> } else { Some(b.project_id.clone()) },
                    "capsule_id": if b.capsule_id.is_empty() { None::<String> } else { Some(b.capsule_id.clone()) },
                    "task_id": if b.task_id.is_empty() { None::<String> } else { Some(b.task_id.clone()) },
                }),
            )
        })
        .collect();
    ...
    let existing_blob_ids = state
        .db
        .get_blobs_by_namespace(owner, namespace, &body.filters)
        .await?;
    ...
    for (blob_id, vector) in &results {
        ...
        let metadata = blob_metadata.get(blob_id).unwrap_or(&empty_metadata);
        state
            .db
            .insert_vector(&id, owner, namespace, blob_id, vector, blob_size, metadata)
            .await?;
    }
