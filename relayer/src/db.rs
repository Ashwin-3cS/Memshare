...
use crate::types::{AppError, MemoryQueryFilters, SearchHit};
...
impl VectorDb {
    ...
    pub async fn new(database_url: &str) -> Result<Self, AppError> {
        ...
        let migration_004 = include_str!("../migrations/004_memory_metadata.sql");
    ...
    pub async fn insert_vector(
        ...
        sqlx::query(
            "INSERT INTO vector_entries (id, owner, namespace, blob_id, embedding, blob_size_bytes, metadata)
    ...
    pub async fn search_similar(
        ...
        let rows: Vec<(String, f64, Value)> = sqlx::query_as(
            "SELECT blob_id, (embedding <=> $1)::float8 AS distance, metadata
             ...
             WHERE owner = $2
               ...
               AND ($5::text IS NULL OR metadata->>'capsule_id' = $5)
               AND ($6::text IS NULL OR metadata->>'task_id' = $6)
        ...
        .bind(filters.capsule_id.as_deref())
        ...
        let rows: Vec<(String,)> = sqlx::query_as(
            "SELECT DISTINCT blob_id FROM vector_entries
             WHERE owner = $1
               ...
               AND ($4::text IS NULL OR metadata->>'capsule_id' = $4)
               AND ($5::text IS NULL OR metadata->>'task_id' = $5)",
        )
        .bind(owner)
        .bind(namespace)
        .bind(filters.project_id.as_deref())
    ...
    pub async fn delete_by_blob_id(&self, blob_id: &str) -> Result<u64, AppError> {
        ...
        if rows > 0 {
            tracing::info!(
                "deleted expired blob from DB: blob_id={}, rows={}",
                blob_id,
                rows
            );
        }
