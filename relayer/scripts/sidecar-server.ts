        ...
        const blob = await runExclusiveBySigner(signerAddress, async () => {
            ...
            const registerTx = flow.register({
                ...
                attributes: {
                    ...
                    ...(projectId ? { memwal_project_id: projectId } : {}),
                    ...(capsuleId ? { memwal_capsule_id: capsuleId } : {}),
                    ...(taskId ? { memwal_task_id: taskId } : {}),
                },
            });
            ...
            try {
                ...
                if (projectId) {
                    metaTx.moveCall({
                        ...
                        arguments: [
                            blobArg,
                            metaTx.pure.string("memwal_project_id"),
                            metaTx.pure.string(projectId),
                        ],
                        typeArguments: [],
                    });
                }
                ...
                if (capsuleId) {
                    metaTx.moveCall({
                        ...
                        arguments: [
                            blobArg,
                            metaTx.pure.string("memwal_capsule_id"),
                            metaTx.pure.string(capsuleId),
                        ],
                        typeArguments: [],
                    });
                }
                ...
                if (taskId) {
                    metaTx.moveCall({
                        ...
                        arguments: [
                            blobArg,
                            metaTx.pure.string("memwal_task_id"),
                            metaTx.pure.string(taskId),
                        ],
                        typeArguments: [],
                    });
                }
...
app.post("/walrus/query-blobs", async (req, res) => {
    try {
        const { owner, namespace, packageId, projectId, capsuleId, taskId } = req.body;
        if (!owner) {
        ...
        type BlobMeta = {
            ...
            blobAgentId: string;
            blobProjectId: string;
            blobCapsuleId: string;
            blobTaskId: string;
        };
        ...
        const metas: BlobMeta[] = await mapConcurrent(rawObjs, 5, async (obj) => {
                ...
                if (dynField.data?.content && dynField.data.content.dataType === "moveObject") {
                    ...
                    if (Array.isArray(contents)) {
                        for (const entry of contents) {
                            ...
                            if (key === "memwal_project_id") blobProjectId = value;
                            if (key === "memwal_capsule_id") blobCapsuleId = value;
                            if (key === "memwal_task_id") blobTaskId = value;
                        }
                    }
                }
            } catch {
            ...
            return { ...obj, blobNamespace, blobOwner, blobPackageId, blobAgentId, blobProjectId, blobCapsuleId, blobTaskId };
        });

        // Step 3: Filter + convert blob IDs
        const blobs: { blobId: string; objectId: string; namespace: string; packageId: string; agentId: string; projectId: string; capsuleId: string; taskId: string }[] = [];
        ...
        for (const meta of metas) {
            ...
            if (projectId && meta.blobProjectId !== projectId) continue;
            if (capsuleId && meta.blobCapsuleId !== capsuleId) continue;
            if (taskId && meta.blobTaskId !== taskId) continue;
            ...
            if (meta.rawBlobId) {
                ...
                blobs.push({
                    ...
                    projectId: meta.blobProjectId,
                    capsuleId: meta.blobCapsuleId,
                    taskId: meta.blobTaskId,
                });
            }
        }
